...
namespace TournamentsXPanded.Patches.TournamentFightMissionControllerClass
{
    public class StartMatch : PatchBase<StartMatch>
    {
        ...
        {
            TournamentXPandedTournamentBehavior._match = match;
            TournamentXPandedTournamentBehavior.firstKiller = null;
            TournamentXPandedTournamentBehavior.achievements = new Dictionary<TournamentParticipant, ParticipantAchievements>();
            foreach (var p in match.Participants)
            {
                TournamentXPandedTournamentBehavior.achievements.Add(p, new ParticipantAchievements());
            }
            return true;
        }
    }
